...
_CRM_CATEGORY_HINTS = {
    ...
    "handle_time": (
        ...
        "    result = sum(vals) if ('total' in _q or 'sum' in _q) and vals else (statistics.mean(vals) if vals else None). "
    ...
    "best_region_identification": (
        ...
        "_is_lowest = any(w in _q for w in ['lowest','least','worst','minimum','fewest']). "
    ...
    "sales_insight_mining": (
        ...
        "  elif any(w in _q for w in ['lead source','leadsource','source']): _gf=next((f for f in ['LeadSource','Source','Channel','ReferralSource'] if data and data[0].get(f)), 'LeadSource'). "
        ...
        "    _is_min_si = any(w in _q for w in ['lowest','least','worst','min','minimum','fewest']). "
