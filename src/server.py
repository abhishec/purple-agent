...
_CRM_CATEGORY_HINTS = {
    ...
    "conversion_rate_comprehension": (
        ...
        "Step 1 — check for pre-computed rate field (HIGHEST PRIORITY): "
        "  _rf = next((f for f in ['ConversionRate','conversion_rate','Rate','ConvertedRate'] if data and data[0].get(f) is not None), None). "
        "  If _rf found: v=float(data[0].get(_rf)); print(round(v*100,2) if v < 1.0 else round(v,2)). STOP. "
        "Step 2 — find converted records: "
