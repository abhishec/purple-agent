...
_CRM_CATEGORY_HINTS = {
    "monthly_trend_analysis": (
        ...
        "_is_sum_mode = any(w in _q for w in ['revenue','amount','value']) or any(p in _q for p in ['total revenue','total amount','total sales','total value','sales amount','deal value']).  # SUM: financial terms only; 'total'/'sales' alone → COUNT (avoids 'total cases'/'sales leads' false match) "
    ...
    "sales_amount_understanding": (
        ...
        "  filtered=list(data). "
        "  if _stage_sa: filtered=[r for r in filtered if r.get('StageName','').lower()==_stage_sa.lower()]. "
        "  if _yr_sa: filtered=[r for r in filtered if _safe_date(r.get('CloseDate') or r.get('CreatedDate')) and _safe_date(r.get('CloseDate') or r.get('CreatedDate')).year==_yr_sa]. "
        "  elif cutoff: filtered=[r for r in filtered if _safe_date(r.get('CloseDate')) and _safe_date(r.get('CloseDate'))>=cutoff].  # date cutoff only if no year filter "
    ...
    "handle_time": (
        ...
        "    raw = (max(durations) if _is_max_ht else (min(durations) if _is_min_ht else (statistics.median(durations) if 'median' in _q else statistics.mean(durations)))) if durations else None. "
    ...
    "conversion_rate_comprehension": (
        ...
        "  _df_cr = lambda r: _safe_date(r.get('ConvertedDate') or r.get('CreatedDate') or r.get('ActivityDate')). "
    ...
    "best_region_identification": (
        ...
        "  rows=list(data). "
        "  if _stage: rows=[r for r in rows if r.get('StageName','').lower()==_stage.lower()]. "
        "  if _yr: rows=[r for r in rows if _safe_date(r.get('CloseDate') or r.get('CreatedDate')) and _safe_date(r.get('CloseDate') or r.get('CreatedDate')).year==_yr]. "
        ...
        "if any(w in _q for w in ['revenue','amount','value','sales','best performance']):  # AMOUNT mode "
        "  _af=next((f for f in ['Amount','Revenue','TotalAmount','SalesAmount','Value'] if data and data[0].get(f) is not None), 'Amount'). "
    ...
    "activity_priority": (
        ...
        "  filtered.sort(key=lambda r: (_prio_order.get(r.get('Priority',''), 9), _safe_date(r.get('ActivityDate') or r.get('DueDate')) or dt.max)). "
    ...
    "sales_cycle_understanding": (
        ...
        "  if _yr_sc: rows=[r for r in rows if _safe_date(r.get('CloseDate') or r.get('CreatedDate')) and _safe_date(r.get('CloseDate') or r.get('CreatedDate')).year==_yr_sc]. "
        "durations = []. "
        "for r in rows: "
        ...
        "agg_days = (max(durations) if _is_max_sc else (min(durations) if _is_min_sc else (statistics.median(durations) if 'median' in _q else statistics.mean(durations)))) if durations else None. "
    ...
    "sales_insight_mining": (
        ...
        "Step 3 — aggregate: "
        "  if any(w in _q for w in ['revenue','amount','value','sales','total']): "
        "    _af=next((f for f in ['Amount','Revenue','TotalAmount','SalesAmount','Value'] if data and data[0].get(f) is not None), 'Amount'). "
    ...
    "top_issue_identification": (
        ...
        "_q = prompt_text.lower(). "
        "Step 0 — pre-filter from prompt_text (year filter): "
