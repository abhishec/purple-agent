...
_CRM_CATEGORY_HINTS = {
    "monthly_trend_analysis": (
        ...
        "If question specifies a time period (e.g., 'in 2024', 'in Q1', 'last year'): filter before grouping: "
        "  rows = [r for r in data if _safe_date(r.get('CreatedDate') or r.get('CloseDate') or r.get('Date')) and _safe_date(r.get('CreatedDate') or r.get('CloseDate') or r.get('Date')).year == 2024]  # year filter. "
        "  rows = data  # no filter (all time). "
        "SUM mode (revenue/sales/amount questions): "
        "  _af = next((f for f in ['Amount','Revenue','TotalAmount','SalesAmount','Value'] if data and data[0].get(f) is not None), 'Amount'). "
        "  monthly = defaultdict(float). "
        "  for r in rows: d = _safe_date(r.get('CreatedDate')) or _safe_date(r.get('CloseDate')) or _safe_date(r.get('Date')) or _safe_date(r.get('ActivityDate')). "
        "  if d: v=r.get(_af); monthly[d.strftime('%B')] += float(v) if v is not None else 0. "
        "  print(max(monthly, key=monthly.get)) if monthly else print(None). "
        "COUNT mode (volume/count/most-records questions): "
        "  monthly = defaultdict(int). "
        "  for r in rows: d = _safe_date(r.get('CreatedDate')) or _safe_date(r.get('CloseDate')) or _safe_date(r.get('Date')) or _safe_date(r.get('ActivityDate')). "
        "  if d: monthly[d.strftime('%B')] += 1. "
        "  print(max(monthly, key=monthly.get)) if monthly else print(None). "
        "Use min() instead of max() if question asks for lowest/minimum."
    ),
    "lead_routing": (
    ...
    "best_region_identification": (
        ...
        "If question specifies a filter (stage, year, status): pre-filter first: "
        "  rows = [r for r in data if r.get('StageName') == 'Closed Won']  # example stage filter. "
        "  rows = data  # no filter. "
        "Region field: _rf = next((f for f in ['Region','State','Territory','BillingState','BillingCountry','Country'] if data and data[0].get(f)), 'Region'). "
        "Choose mode based on question: "
        "  COUNT mode ('most deals/cases/records', 'highest number of'): "
        "    c=Counter(r.get(_rf) for r in rows if r.get(_rf)); print(c.most_common(1)[0][0] if c else None). "
        "  AMOUNT mode ('highest revenue/sales/value', 'best performance'): "
        "    _af = next((f for f in ['Amount','Revenue','TotalAmount','SalesAmount','Value'] if data and data[0].get(f) is not None), 'Amount'). "
        "    grouped = defaultdict(float). "
        "    for r in rows: k=r.get(_rf); v=r.get(_af). "
        "    if k: grouped[k] += float(v) if v is not None else 0. "
        "    print(max(grouped, key=grouped.get) if grouped else None) — or min() if question asks lowest. "
    ...
    "sales_insight_mining": (
        ...
        "If question specifies a stage/status/period: pre-filter first: "
        "  rows = [r for r in data if r.get('StageName') == 'Closed Won']  # won deals only. "
        "  rows = [r for r in data if _safe_date(r.get('CloseDate')) and _safe_date(r.get('CloseDate')).year == 2024]  # year filter. "
        "  rows = data  # no filter. "
        ...
        "Count-based (most records, highest volume by group): "
        "  c = Counter(r.get(_gf) for r in rows if r.get(_gf)); print(c.most_common(1)[0][0] if c else None). "
