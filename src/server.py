...
_CRM_CATEGORY_HINTS = {
    ...
    "sales_insight_mining": (
        ...
        "Determine groupby field from question (agent→OwnerId/SalesRep, product→ProductName, region→BillingState/Region). "
        "Amount-based (highest revenue/sales): "
        "_af = next((f for f in ['Amount','Revenue','TotalAmount','SalesAmount','Value'] if data and data[0].get(f) is not None), 'Amount'). "
        "groups = defaultdict(float). "
        "for r in data: k=r.get('OwnerId')  # REPLACE with actual groupby field (OwnerId/SalesRep/Region/ProductName etc.); v=r.get(_af); "
        ...
        "c = Counter(r.get('OwnerId') for r in data if r.get('OwnerId'))  # REPLACE field; c.most_common(1)[0][0] if c else None. "
