...
_CRM_CATEGORY_HINTS = {
    "monthly_trend_analysis": (
        ...
        "    if d and v not in (None,'','N/A','n/a'): monthly[d.strftime('%B')] += _safe_num(v).  # Bug 119 "
    ...
    "lead_routing": (
        ...
        "  elif int(_safe_num(lead.get('Score'))) >= 70: print('Enterprise Team')  # example — Bug 117: use _safe_num not float() "
    ...
    "transfer_count": (
        ...
        "   If found: _tv = next((r.get(_tf) for r in data if r.get(_tf) is not None), None). print(int(_safe_num(_tv))) if _tv is not None else print(0).  # Bug 118: data[0] may be None for _tf; Bug 119: _safe_num avoids ValueError "
    ...
    "sales_amount_understanding": (
        ...
        "vals = [_safe_num(r.get(_af)) for r in filtered if r.get(_af) not in (None,'','N/A','n/a')].  # Bug 119: _safe_num handles comma-numbers/N/A; filter prevents 0-skew "
    ...
    "handle_time": (
        ...
        "    vals = [_safe_num(r.get(_ht_field)) for r in rows if r.get(_ht_field) not in (None,'','N/A','n/a')].  # Bug 119 "
    ...
    "conversion_rate_comprehension": (
        ...
        "    _rv = [_safe_num(r.get(_rf)) for r in rows_cr if r.get(_rf) not in (None,'','N/A','n/a')].  # Bug 119 "
    ...
    "best_region_identification": (
        ...
        "    if k and v not in (None,'','N/A','n/a'): grouped[k]+=_safe_num(v).  # Bug 119: _safe_num avoids ValueError "
    ...
    "sales_insight_mining": (
        ...
        "      if k and v not in (None,'','N/A','n/a'): groups[k]+=_safe_num(v).  # Bug 119 "
